...
export default defineTool({
  ...
  inputSchema: {
    query: z.string().describe("Word or phrase to look for in the emotion, scenario, or text pattern."),
  ...
  handler: async ({ query, limit }, ctx) => {
    ...
    const { data, error } = await supabase
      ...
      .select("emotion, emotion_category, communication_state, scenario, text_pattern, user_need, ai_response_strategy")
